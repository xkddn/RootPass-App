...
Ce projet a été réalisé avec l'assistance d'outils IA pour accélérer certaines phases du développement :

- **Claude Code :** Sollicité principalement comme assistant créatif et technique sur le front-end. Il a aidé à définir la direction artistique (choix de la palette de couleurs, des polices, idées d'UI/UX) et a fourni un support précieux pour le débogage quotidien.
- **Gemini :** Utilisé pour la gestion et la planification du projet. Il a permis de structurer la roadmap, d'organiser le déroulement des différentes phases de développement et de valider les choix architecturaux globaux.

_Note : L'IA a agi comme un outil de productivité. L'intégralité de la logique métier, du modèle de sécurité, du chiffrement et de la validation du code reste strictement humaine (en référence aux documentations officielles) afin de garantir un niveau de fiabilité absolu._
