...
export function addAccount(accountData) {
  const { title, url, login, password, category, custom_fields, totp_secret } = accountData
  ...
  const stmt = getDb().prepare(`
    INSERT INTO accounts (id, title, url, login, password, category, is_favorite, custom_fields, created_at, password_updated_at, totp_secret)
    VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)
  `)
...
export function updateAccount(id, accountData) {
  ...
  if (passwordUpdatedAt) {
    getDb()
      .prepare(
        `UPDATE accounts SET title=?, url=?, login=?, password=?, category=?, custom_fields=?, password_updated_at=?, totp_secret=? WHERE id=?`
      )
      .run(
        title,
        encryptedUrl,
    ...
    getDb()
      .prepare(
        `UPDATE accounts SET title=?, url=?, login=?, password=?, category=?, custom_fields=?, totp_secret=? WHERE id=?`
      )
      .run(
        title,
        encryptedUrl,
