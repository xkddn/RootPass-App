...
import { X, Dices, Plus, Trash2, Save, Eye, EyeOff, Wand2 } from 'lucide-react'
...
function AddAccountModal({ folders = [], onClose, onSuccess }) {
  ...
  useEffect(() => {
    let cancelled = false
    window.api.getActiveDomain?.().then((domain) => {
      if (cancelled || !domain) return
      setFormData((prev) => (prev.url ? prev : { ...prev, url: domain }))
      ...
      <div className="flex w-full max-w-md flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-(--surface-solid) shadow-2xl shadow-black/60 max-h-[90vh]">
        ...
        <div className="overflow-y-auto p-5">
          <form id="add-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label htmlFor="add-url" className={`${labelClass} flex items-center gap-2`}>
                {t('accountForm.urlLabel')}
                {urlSuggested && formData.url && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-[9px] font-bold normal-case tracking-normal text-emerald-400">
                    <Wand2 className="size-2.5" />
                    {t('accountForm.urlSuggested')}
                  </span>
                )}
              </label>
              <input
