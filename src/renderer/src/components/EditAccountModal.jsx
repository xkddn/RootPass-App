...
/**
 * @param {{ account: object, folders: Array, onClose: () => void, onSuccess: () => void, onDelete: () => void }} props
 */
function EditAccountModal({ account, folders = [], onClose, onSuccess, onDelete }) {
  ...
  const [formData, setFormData] = useState({
    ...
    category: account.category || 'Autres',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await window.api.updateAccount(account.id, formData)
      onSuccess()
      onClose()
    } catch (error) {
      console.error(error)
  ...
  return (
    ...
    >
      <div className="flex w-full max-w-md flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-(--surface-solid) shadow-2xl shadow-black/60 max-h-[90vh]">
        <div className="flex items-center justify-between border-b border-white/[0.05] px-5 py-4">
          <h2 id="edit-modal-title" className="text-[15px] font-bold text-white">
        ...
        <div className="overflow-y-auto p-5">
          <form id="edit-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label htmlFor="edit-login" className={labelClass}>
            ...
            <div className="border-t border-white/[0.05] pt-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className={labelClass}>{t('accountForm.folderLabel')}</label>
                  <CustomSelect
                    value={formData.folder_id || ''}
                    onChange={(val) => setFormData((prev) => ({ ...prev, folder_id: val || null }))}
                    options={[
                      { value: '', label: t('accountForm.noFolder') },
            ...
            <div>
              ...
              <TagInput
                ...
                placeholder={t('accountForm.tagsPlaceholder')}
              />
            </div>

            <div className="border-t border-white/[0.05] pt-4">
              <label htmlFor="edit-totp" className={labelClass}>
        ...
        <div className="flex items-center justify-between border-t border-white/[0.05] px-5 py-4">
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl px-3 py-2 text-[13px] font-medium text-zinc-700 transition-colors hover:bg-red-500/10 hover:text-red-400 ${FOCUS_RING}`}
          >
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {isSubmitting ? t('accountForm.updating') : t('accountForm.updatePassword')}
            </button>
          </div>
        </div>
      </div>
